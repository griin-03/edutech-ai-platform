...
import { 
  ...
  ShieldCheck, Sun, Moon, Users, Database // Icon mới cho Admin
} from "lucide-react";
...
export default function MobileAppDashboard() {
  ...
  if (status === "loading" || isLoading) {
    return (
      <div className="min-h-screen bg-stone-50 dark:bg-stone-950 flex flex-col items-center justify-center max-w-md mx-auto">
        ...
        <p className="text-stone-500 font-bold">Đang tải dữ liệu thật...</p>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-stone-50 dark:bg-stone-950 pb-24 font-sans max-w-md mx-auto relative shadow-2xl overflow-hidden animate-in fade-in duration-500">
      ...
      <div className="px-5 -mt-16 relative z-20 space-y-4">
        ...
        <div className="bg-white dark:bg-stone-900 rounded-2xl p-4 shadow-xl shadow-stone-900/5 border border-stone-100 dark:border-stone-800 flex justify-between items-center">
          <div className="flex items-center gap-3">
             <div className="p-2.5 bg-amber-100 dark:bg-amber-900/30 text-amber-600 rounded-xl">
               {isAdmin ? <Database size={20} className="text-indigo-600" /> : <Wallet size={20} />}
             </div>
             <div>
          ...
          <div className={`px-3 py-1.5 rounded-lg text-xs font-bold flex items-center gap-1.5 ${isAdmin ? "bg-indigo-100 text-indigo-700" : appData?.userInfo?.isPro ? "bg-gradient-to-r from-orange-400 to-amber-500 text-white shadow-md" : "bg-stone-100 dark:bg-stone-800 text-stone-500"}`}>
             {isAdmin ? <ShieldCheck size={14} /> : <Crown size={14} />} {isAdmin ? "SYSTEM" : appData?.userInfo?.isPro ? "PRO VIP" : "Free"}
          </div>
        </div>
        ...
        {activeTab === "home" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
            ...
            {/* AI MENTOR / SYSTEM LOG */}
            <div className={`${isAdmin ? "bg-indigo-50 dark:bg-indigo-900/20 border-indigo-200 dark:border-indigo-900/50" : "bg-blue-50 dark:bg-blue-900/20 border-blue-200 dark:border-blue-900/50"} border rounded-2xl p-4 flex gap-4`}>
              <div className="mt-1"><Sparkles size={24} className={isAdmin ? "text-indigo-500" : "text-blue-500"} /></div>
              <div>
                <h4 className={`font-bold text-sm ${isAdmin ? "text-indigo-900 dark:text-indigo-400" : "text-blue-900 dark:text-blue-400"}`}>
                  {isAdmin ? "Báo cáo Hệ thống" : "Gợi ý từ AI Mentor"}
                </h4>
                <p className={`text-xs mt-1 leading-relaxed ${isAdmin ? "text-indigo-700 dark:text-indigo-300" : "text-blue-700 dark:text-blue-300"}`}>
                  {isAdmin 
                    ? `Hệ thống ghi nhận tổng cộng ${appData?.adminStats?.totalExams || 0} lượt nộp bài. Server hoạt động ổn định, không có cảnh báo bảo mật.` 
                    : `Dựa trên ${appData?.scores?.length || 0} bài thi gần nhất, thuật toán nhận thấy bạn đang làm rất tốt. Hãy duy trì nhịp độ nhé!`}
                </p>
              </div>
            </div>

            {/* NHIỆM VỤ (Ẩn nếu là Admin) */}
            {!isAdmin && (
              <div>
        ...
        {/* TAB 2: HỌC TẬP (Nội dung Admin sẽ khác) */}
        {activeTab === "study" && (
          <div className="space-y-6 animate-in fade-in slide-in-from-right-4">
             {isAdmin ? (
               <div>
                  ...
                  <p className="text-xs text-stone-500 bg-white dark:bg-stone-900 p-4 rounded-xl border border-dashed dark:border-stone-800 text-center">Hiện không có yêu cầu nâng cấp Giáo viên nào.</p>
               </div>
             ) : (
               <>
                 <div>
        ...
        {/* TAB 3: BẢNG ĐIỂM (HIỂN THỊ CHUNG) */}
        {activeTab === "scores" && (
          <div className="space-y-4 animate-in fade-in slide-in-from-right-4">
            <h3 className="font-bold text-stone-800 dark:text-stone-100">{isAdmin ? "Hoạt động thi toàn hệ thống" : isTeacher ? "Lịch sử nộp bài của học sinh" : "Lịch sử điểm số cá nhân"}</h3>
      ...
      {/* BOTTOM NAVIGATION DYNAMIC THAY ĐỔI MÀU THEO ROLE */}
      <div className="fixed bottom-0 left-0 right-0 max-w-md mx-auto bg-white dark:bg-stone-900 border-t border-stone-200 dark:border-stone-800 pb-safe z-50 rounded-t-3xl shadow-[0_-10px_40px_rgba(0,0,0,0.05)]">
        <div className="flex justify-between items-center px-6 py-3">
          {[
            ...
            { id: "study", icon: isAdmin ? ShieldCheck : BookOpen, label: isAdmin ? "Kiểm duyệt" : "Học tập" },
            { id: "scores", icon: isAdmin ? Users : Brain, label: isAdmin ? "Hoạt động" : "Bảng điểm" },
            { id: "profile", icon: User, label: "Cá nhân" },
          ].map((item) => (
            <button key={item.id} onClick={() => setActiveTab(item.id)} className={`flex flex-col items-center gap-1.5 transition-all duration-300 ${activeTab === item.id ? (isAdmin ? "text-indigo-600 scale-110" : isTeacher ? "text-emerald-600 scale-110" : "text-amber-600 scale-110") : "text-stone-400"}`}>
